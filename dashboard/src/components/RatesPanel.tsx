...
export function RatesPanel({ stats }: { stats: Stats }) {
  ...
  return (
    <section aria-label="Throughput by upstream">
      ...
      {rows.length === 0 ? (
        <p className="text-muted-foreground text-sm">No answers yet. Send a message and the rates fill in.</p>
      ) : (
      ...
      {Object.keys(stats.fallbacks).length > 0 && (
        <p className="text-muted-foreground mt-2 text-xs">
          Fallbacks: {Object.entries(stats.fallbacks).map(([k, v]) => `${k} (${v})`).join(", ")}
        </p>
      )}
    </section>
  )
