...
export function StatusStrip({ stats }: { stats: Stats }) {
  ...
  const trend = stats.recent.filter((r) => r.served_by === "cluster" && r.decode_tps != null).slice(-20).map((r) => r.decode_tps as number)
    ...
    <section aria-label="Cluster front panel" className="border-border grid gap-4 border-y py-4 md:grid-cols-[1fr_auto] md:items-center">
      ...
      <div className="flex flex-wrap items-end gap-6">
        <div className="flex items-end gap-3">
          ...
          <Sparkline values={trend} color="#199e70" />
        </div>
        <div className="min-w-36">
          <div className="text-2xl leading-none font-medium">{stats.pct_local ?? 0}%</div>
          <div
            className="bg-muted mt-1.5 h-1.5 w-full overflow-hidden rounded-full"
            role="meter"
            aria-valuenow={stats.pct_local ?? 0}
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label="Share of answers served on the Pis"
          >
            <div className="h-full rounded-full" style={{ width: `${stats.pct_local ?? 0}%`, backgroundColor: "#199e70" }} />
          </div>
          <div className="text-muted-foreground mt-1 text-sm">served on the Pis</div>
        </div>
        {cloud && (
          <div>
