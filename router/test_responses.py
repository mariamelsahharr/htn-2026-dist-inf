...
SHELL_TOOL = {
    ...
    "strict": False,
    "parameters": {"type": "object", "properties": {"command": {"type": "array"}}, "required": ["command"]},
}
PATCH_TOOL = {
    "type": "custom",
    "name": "apply_patch",
...
def test_function_tool_translates_verbatim():
    ...
    assert chat["tools"] == [
        {
            "type": "function",
            "function": {"name": "shell", "description": "Run a command", "parameters": SHELL_TOOL["parameters"]},
        }
    ]
    assert custom == {}
...
def test_custom_tool_call_comes_back_as_custom_tool_call_item():
    ...
    done = next(e for e in events(b.finish()) if e["type"] == "response.output_item.done")
